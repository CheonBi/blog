...
export function createClientModuleProxy(moduleId) {
  const cachedReferences = Object.create(null)
  return new Proxy(
    {},
    {
      get(_, name) {
        if (cachedReferences[name]) return cachedReferences[name]
        return (cachedReferences[name] = registerClientReference(
          () => {
            throw new Error('...')
          },
          moduleId,
...
```mermaid
flowchart LR
    A["같은 모듈"] --> B[RSC server bundle]
    A --> C[SSR bundle]
    A --> D[Client bundle]
    B -->|stub만| E[Flight Payload]
    E -->|소비| F[SSR이 HTML 생성]
    C -->|구현| F
    D -->|hydrate| F
```
...
```mermaid
flowchart TD
  A["'use client' module"] --> B[RSC layer: stub]
  A --> C[Client layer: 원본]
  A --> D[SSR layer: 원본]

  B --> E[serializeClientReference]
  E --> F[$L token in Flight]
  F --> G[Client: parseModelString]
  G --> H[React.lazy + chunk load]
  H --> I[실제 컴포넌트]
```
